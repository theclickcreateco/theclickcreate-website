...
export default function Pricing() {
  ...
  const plans = [
    {
      ...
      duration: "4–5 weeks",
      features: [
        "Lead Generation website rebuild (B2B focused)",
  ...
  const addons = [
    {
      name: "Product catalog design (PDF)",
      price: "$200",
      icon: <FileText className="w-6 h-6" />,
    },
    {
      name: "Certification landing pages",
      price: "$150 / page",
      icon: <Award className="w-6 h-6" />,
    },
    {
      name: "WhatsApp automation & CRM setup",
      price: "$250",
      icon: <MessageSquare className="w-6 h-6" />,
    },
    {
      name: "Shopify catalog integration",
      price: "Custom Quote",
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      <AnimationWrapper>
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Flexible engagement tiers designed to match your current needs and
            future growth goals.
          </p>
        </div>
      </AnimationWrapper>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((plan, i) => (
          <AnimationWrapper
            key={i}
            delay={i * 0.1}
            className={`relative flex flex-col bg-card border ${plan.popular ? "border-blue-500 border-2 shadow-2xl shadow-blue-500/10" : "border-border"} rounded-2xl p-8 hover:shadow-xl transition-shadow card-gradient-hover`}
          >
            {plan.popular && (
            ...
            <p className="text-sm text-muted-foreground mb-8">
              Duration:{" "}
              <span className="text-foreground font-bold">{plan.duration}</span>
            </p>
      ...
      <div className="mt-20 max-w-4xl mx-auto">
        ...
        <p className="text-center text-muted-foreground mb-8">
          Select multiple add-ons to enhance your package.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {addons.map((addon, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-3 rounded-full ${isSelected ? "bg-blue-500 text-white" : "bg-secondary text-muted-foreground"}`}
                >
                  {addon.icon}
                </div>
                ...
                <div
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${isSelected ? "border-blue-500 bg-blue-500" : "border-muted-foreground"}`}
                >
        ...
        {selectedAddons.length > 0 && (
          <div className="flex justify-center animate-in fade-in slide-in-from-bottom-4">
            ...
            >
              Proceed with Selected Add-Ons ({selectedAddons.length}){" "}
